    ...
    <main className="site-shell">
      ...
      <section className="workspace-grid">
        <div className="graph-panel">
          ...
          <div className="replay-controls"><button onClick={() => { desktopFollowingRef.current = false; setPlaying((value) => !value); setFollowing(false); }} type="button">{playing ? "Pause" : "Replay"}</button><button className={following ? "active" : ""} onClick={() => { desktopFollowingRef.current = true; setFollowing(true); setPlaying(false); setCursor(snapshot.sequence); }} type="button">Follow latest</button><input aria-label="Replay position" min={0} max={snapshot.sequence} value={cursor} onChange={(event) => { desktopFollowingRef.current = false; setCursor(Number(event.target.value)); setFollowing(false); setPlaying(false); }} type="range" /><output>#{cursor}</output></div>
