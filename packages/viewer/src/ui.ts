  ...
  <script>
    (function () {
      ...
      function usageAtCursor() {
        ...
        var values=Object.keys(unique).map(function(key){return unique[key];});var models={};var totals={invocations:values.length,tokenInvocations:0,pricedInvocations:0,totalTokens:0,costUsd:0};
        values.forEach(function(node){var usage=node.usage||{};var tokens=usageTokenTotal(usage);if(tokens!=null){totals.tokenInvocations++;totals.totalTokens+=tokens;}if(usage.costUsd!=null){totals.pricedInvocations++;totals.costUsd+=usage.costUsd;}var key=(usage.provider||"")+"\u0000"+(usage.model||"");var model=models[key]||(models[key]={label:modelLabel(usage),invocations:0,tokens:0,cost:0,tokenInvocations:0,pricedInvocations:0});model.invocations++;if(tokens!=null){model.tokenInvocations++;model.tokens+=tokens;}if(usage.costUsd!=null){model.pricedInvocations++;model.cost+=usage.costUsd;}});
      ...
      function renderUsage() {
        var usage=usageAtCursor();var top=usage.models[0];var tokenValue=usage.tokenInvocations?fmtTokens(usage.totalTokens):"unreported";var costValue=usage.pricedInvocations?fmtCost(usage.costUsd):"unreported";if(usage.pricedInvocations&&usage.pricedInvocations<usage.invocations)costValue+="+";
        var cards=[[usage.invocations,"Invocations",usage.invocations?"agents, subagents, and model-backed evaluators":"none recorded"],[top?top.label:"unreported","Models",usage.models.length?usage.models.length+" distinct configuration"+(usage.models.length===1?"":"s"):"adapter did not identify a model"],[tokenValue,"Tokens",usage.tokenInvocations+" / "+usage.invocations+" invocations reported"],[costValue,"Cost",usage.pricedInvocations+" / "+usage.invocations+" invocations priced"]];
      ...
      function usageDetails(usage,invocationId,parentInvocationId) {
        if(!usage&&!invocationId&&!parentInvocationId)return "";var rows=[];if(invocationId)rows.push(["Invocation",invocationId]);if(parentInvocationId)rows.push(["Parent",parentInvocationId]);if(usage){if(usage.provider)rows.push(["Provider",usage.provider]);if(usage.model)rows.push(["Model",usage.model]);if(usage.inputTokens!=null)rows.push(["Input tokens",fmtNumber(usage.inputTokens)]);if(usage.cachedInputTokens!=null)rows.push(["Cached input",fmtNumber(usage.cachedInputTokens)]);if(usage.outputTokens!=null)rows.push(["Output tokens",fmtNumber(usage.outputTokens)]);if(usage.reasoningTokens!=null)rows.push(["Reasoning tokens",fmtNumber(usage.reasoningTokens)]);var total=usageTokenTotal(usage);if(total!=null)rows.push(["Total tokens",fmtNumber(total)]);rows.push(["Cost",usage.costUsd==null?"unreported":fmtCost(usage.costUsd)+(usage.costSource?" · "+usage.costSource:"")]);if(usage.pricingVersion)rows.push(["Pricing",usage.pricingVersion]);}return rows.map(function(row){return '<div class="artifact"><span>'+esc(row[0])+'</span><span class="artifact-kind" title="'+esc(row[1])+'">'+esc(row[1])+'</span></div>';}).join("");
      }
      ...
      function renderDetail(experiments) {
        ...
        var agentHtml=hasResult&&exp.contributors.length?exp.contributors.map(function(a){var facts=[a.role];if(a.usage&&a.usage.model)facts.push(modelLabel(a.usage));var tokens=usageTokenTotal(a.usage);if(tokens!=null)facts.push(fmtTokens(tokens)+' tokens');if(a.usage&&a.usage.costUsd!=null)facts.push(fmtCost(a.usage.costUsd));return '<div class="agent-row"><div><div class="agent-name">'+esc(a.agentId)+'</div><div class="agent-role">'+esc(facts.join(' · '))+'</div><p>'+esc(a.summary)+'</p></div>'+statusBadge(a.status)+'</div>';}).join(""):hasResult&&exp.agentSummary?'<div class="detail-summary">'+esc(exp.agentSummary)+'</div>':'';
