...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .prompt-summary { margin-top: 8px; padding: 10px; border: 1px solid var(--line); border-radius: 7px; background: rgba(255,255,255,.02); }
    .prompt-summary summary { color: var(--amber); font: 9px ui-monospace, SFMono-Regular, Menlo, monospace; cursor: pointer; }
    .prompt-summary code { display: block; margin-top: 7px; color: var(--dim); font: 8px/1.4 ui-monospace, SFMono-Regular, Menlo, monospace; overflow-wrap: anywhere; }
    .prompt-summary pre { max-height: 260px; margin: 9px 0 0; padding: 9px; overflow: auto; border-radius: 5px; background: #090c0d; color: var(--muted); font: 9px/1.5 ui-monospace, SFMono-Regular, Menlo, monospace; white-space: pre-wrap; }
...
<body>
  ...
  <script>
    (function () {
      ...
      var modelLabel = function (usage) { if(!usage)return "unreported"; var label=[usage.provider,usage.model].filter(Boolean).join(" / ")||"unreported";return usage.reasoningEffort?label+" · "+usage.reasoningEffort+" effort":label; };
      ...
      function usageAtCursor() {
        ...
        values.forEach(function(node){var usage=node.usage||{};var tokens=usageTokenTotal(usage);if(tokens!=null){totals.tokenInvocations++;totals.totalTokens+=tokens;}if(usage.costUsd!=null){totals.pricedInvocations++;totals.costUsd+=usage.costUsd;}if(usage.billingMode&&totals.billingModes.indexOf(usage.billingMode)<0)totals.billingModes.push(usage.billingMode);var key=(usage.provider||"")+"\u0000"+(usage.model||"")+"\u0000"+(usage.reasoningEffort||"");var model=models[key]||(models[key]={label:modelLabel(usage),invocations:0,tokens:0,cost:0,tokenInvocations:0,pricedInvocations:0,billingModes:[]});model.invocations++;if(tokens!=null){model.tokenInvocations++;model.tokens+=tokens;}if(usage.costUsd!=null){model.pricedInvocations++;model.cost+=usage.costUsd;}if(usage.billingMode&&model.billingModes.indexOf(usage.billingMode)<0)model.billingModes.push(usage.billingMode);});
      ...
      function usageDetails(usage,invocationId,parentInvocationId) {
        if(!usage&&!invocationId&&!parentInvocationId)return "";var rows=[];if(invocationId)rows.push(["Invocation",invocationId]);if(parentInvocationId)rows.push(["Parent",parentInvocationId]);if(usage){if(usage.provider)rows.push(["Provider",usage.provider]);if(usage.model)rows.push(["Model",usage.model+(usage.identitySource?" · "+usage.identitySource:"")]);if(usage.reasoningEffort)rows.push(["Effort",usage.reasoningEffort]);if(usage.inputTokens!=null)rows.push(["Input tokens",fmtNumber(usage.inputTokens)]);if(usage.cachedInputTokens!=null)rows.push(["Cached input",fmtNumber(usage.cachedInputTokens)]);if(usage.outputTokens!=null)rows.push(["Output tokens",fmtNumber(usage.outputTokens)]);if(usage.reasoningTokens!=null)rows.push(["Reasoning tokens",fmtNumber(usage.reasoningTokens)]);var total=usageTokenTotal(usage);if(total!=null)rows.push(["Total tokens",fmtNumber(total)]);rows.push(["Billing",billingValue(usage)]);if(usage.costUsd!=null&&usage.costSource==="estimated")rows.push(["API equivalent",fmtCost(usage.costUsd)+" · estimate"]);if(usage.pricingVersion)rows.push(["Pricing",usage.pricingVersion]);}return rows.map(function(row){return '<div class="artifact"><span>'+esc(row[0])+'</span><span class="artifact-kind" title="'+esc(row[1])+'">'+esc(row[1])+'</span></div>';}).join("");
      }
      ...
      function promptDetails(manifest) {
        if(!manifest||!Array.isArray(manifest.layers))return "";var provider=(manifest.providerContext&&manifest.providerContext.modelProvider)||manifest.provider||"unreported";var requestedModel=(manifest.providerContext&&manifest.providerContext.requestedModel)||manifest.model||"provider default";var model=(manifest.providerContext&&manifest.providerContext.actualModel)||"unreported";var requestedEffort=(manifest.providerContext&&manifest.providerContext.requestedReasoningEffort)||manifest.reasoningEffort||"provider default";var effort=(manifest.providerContext&&manifest.providerContext.reasoningEffort)||"unreported";
        var skills=Array.isArray(manifest.skills)&&manifest.skills.length?manifest.skills.map(function(skill){return skill.name;}).join(" · "):"none bound";
        var layers=manifest.layers.map(function(layer){return '<details class="prompt-summary"><summary>'+esc(layer.kind+' · '+layer.id)+'</summary><code>'+esc(layer.source)+(layer.version?' · v'+esc(layer.version):'')+' · '+esc(String(layer.sha256||'').slice(0,12))+'</code><pre>'+esc(layer.content||'')+'</pre></details>';}).join("");
        return '<div class="section-label">Prompt &amp; context</div><div class="artifact"><span>Adapter</span><span class="artifact-kind">'+esc(manifest.adapter||"unreported")+'</span></div><div class="artifact"><span>Provider</span><span class="artifact-kind">'+esc(provider)+'</span></div><div class="artifact"><span>Requested model</span><span class="artifact-kind">'+esc(requestedModel)+'</span></div><div class="artifact"><span>Resolved model</span><span class="artifact-kind">'+esc(model)+'</span></div><div class="artifact"><span>Requested effort</span><span class="artifact-kind">'+esc(requestedEffort)+'</span></div><div class="artifact"><span>Resolved effort</span><span class="artifact-kind">'+esc(effort)+'</span></div><div class="artifact"><span>Skills</span><span class="artifact-kind">'+esc(skills)+'</span></div>'+layers;
      }
