...
test("game spec evaluator validates frozen claims and preserves the concept", async () => {
  ...
  try {
    ...
    const campaign: Campaign = { apiVersion: "gamefactory.dev/v1", id: "spec", objective: "spec", projectRoot: ".", workflow: "fixture", requires: [], budget: { maximumExperiments: 3 }, parameters: { gameSpec: { path: "GAME_SPEC.json", conceptPath: "concept.md", projectId: "fixture", maximumConvergencePasses: 3, requiredClaims: ["loop.errand"], requiredSlices: ["first-errand"] } }, acceptance: { primaryMetric: "spec_valid", direction: "maximize" } };
