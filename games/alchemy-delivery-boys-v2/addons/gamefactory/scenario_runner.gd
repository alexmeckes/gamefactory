...
func run_scenario() -> void:
	...
	if not request is Dictionary:
		...
		return
	var packed = load(request.get("path", "res://game/main.tscn"))
	if not packed is PackedScene:
		fail("factory.scene", "Cannot load the scenario scene.")
		return
	var subject: Node = packed.instantiate()
	root.add_child(subject)
	var parameters: Dictionary = request.get("parameters", {})
	var ticks: int = maxi(1, int(parameters.get("ticks", 900)))
	...
	var metrics := {"ticks_completed":ticks, "completion":1.0, "simulated_seconds":float(ticks) / float(physics_hz)}
	...
	if subject.has_method("factory_collect"):
		...
		violations.append({"code":"factory.hooks", "message":"Missing factory_collect.", "severity":"error"})
	var artifacts: Array = [{"kind":"telemetry", "path":telemetry_path, "mediaType":"application/x-ndjson", "label":"Deterministic game telemetry"}]
	write_json(output_dir.path_join("result.json"), {
		"status":"pass" if violations.is_empty() else "fail", "metrics":metrics,
