...
func fail(code: String, message: String) -> void:
	if not output_dir.is_empty():
		write_json(output_dir.path_join("result.json"), {"status":"crash", "metrics":{}, "artifacts":[], "violations":[{"code":code, "message":message, "severity":"error"}]})
	push_error(message)
	quit(1)
...
func run_scenario() -> void:
	...
	if not request_file:
		fail("factory.request", "Cannot open scenario request.")
		return
	...
	var ticks: int = maxi(1, int(parameters.get("ticks", 360)))
	...
	if subject.has_method("factory_setup"):
		...
		fail("firefly.factory-hooks", "Missing factory_setup.")
		return
	...
	var metrics := {"ticks_completed":ticks, "simulated_seconds":float(ticks) / float(physics_hz)}
	...
	if subject.has_method("factory_collect"):
		...
		violations.append({"code":"firefly.factory-hooks", "message":"Missing factory_collect.", "severity":"error"})
	var artifacts := [{"kind":"telemetry", "path":telemetry_path, "mediaType":"application/x-ndjson", "label":"Firefly Glassworks deterministic telemetry"}]
